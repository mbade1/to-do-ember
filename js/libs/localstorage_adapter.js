...
(function () {
  ...
  DS.LSSerializer = DS.JSONSerializer.extend({
    serializeHasMany: function (record, json, relationship) {
      ...
      var payloadKey = this.keyForRelationship
        ? this.keyForRelationship(key, "hasMany")
        : key;
  ...
  DS.LSAdapter = DS.Adapter.extend(Ember.Evented, {
    ...
    loadRelationships: function (type, record) {
      ...
      relationships = relationshipNames.belongsTo.concat(
        ...
        if (relationEmbeddedId && foreignAdapter === adapter) {
          recordPromise = recordPromise.then(function (recordPayload) {
            ...
            return promise.then(function (relationRecord) {
              return adapter.addEmbeddedPayload(
                recordPayload,
                relationName,
                relationRecord
              );
    ...
    addEmbeddedPayload: function (
      ...
      var arrayHasIds =
        relationshipRecord.length && relationshipRecord.everyBy("id");
      ...
      if (this.isArray(payload[relationshipName])) {
        payload[relationshipName] = payload[relationshipName].filter(function (
          id
        ) {
          return id;
        });
